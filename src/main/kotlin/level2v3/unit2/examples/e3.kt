...
private fun drawMyObject(gc: Graphics, x: Double, y: Double, tick: Int) {
    ...
    gc.drawOval((x - height/2).roundToInt(), (y - width/2).roundToInt(),  height.roundToInt(), width.roundToInt())

    val circleSize = baseSize * tickK / 10
    gc.color = Pal16.brightRed
    gc.drawOval((x - circleSize/2).roundToInt(), (y - circleSize/2).roundToInt(), circleSize.roundToInt(), circleSize.roundToInt(), fill = true)
}
