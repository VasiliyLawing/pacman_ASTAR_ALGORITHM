...
package level2.unit6.examples.e9.step4
...
private data class Star(var x: Int, var y: Int, var crazyTime: Int?)
...
private fun updateFlyier(flyier: Flyier) {
    ...
    if (flyier.leftBoost > 20)
        flyier.leftBoost = 20.0
}
...
private fun drawStars(g: Graphics, list: List<Star>) {
    ...
    for (o in list) {
        g.color = if (o.crazyTime != null) Pal16.brightRed else Pal16.brightBlue
        g.drawDot(o.x, o.y)
    }
}
...
private fun updateStars(game: Game, wndWidth: Int, wndHeight: Int) {
    if (game.growCounter-- == 0) {
        game.stars.add(Star(0, Random.nextInt(0, wndHeight), crazyTime = null))
        game.growCounter = 5
    }
    ...
    for (star in game.stars) {
        ...
        if (star.crazyTime != null) {
            star.x += Random.nextInt(0, 5)
            star.y += Random.nextInt(-2, 3)

            star.crazyTime = star.crazyTime!! + 1
            if (star.crazyTime == 50) {
                star.crazyTime = null
            }
        }
